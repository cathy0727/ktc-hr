...
RW = {a.strip() for a in env.get("HR_RW","").split(",") if a.strip()}

import re as _re_mail
_EMAIL_OK = _re_mail.compile(r"[A-Za-z0-9._%+\-]+@[A-Za-z0-9.\-]+\.[A-Za-z]{2,}")
def _guard_mail(addr):
    """對外收件信箱守門：格式合法、長度安全、且非 104 系統信箱才放行；否則回 None"""
    a = (addr or "").strip()
    if not a or len(a) > 120 or _EMAIL_OK.fullmatch(a) is None:
        return None
    if "104.com" in a.lower():
        return None
    return a
...
def me_rw(req: Request):
    u = me(req)
    if RW and u not in RW:
        raise HTTPException(403, "唯讀帳號，僅供瀏覽")
    return u
...
def whoami(req: Request):
    u = me(req)
    return {"user": u, "rw": (not RW) or (u in RW)}
...
# ── 題庫解析：Excel 範本（原邏輯抽成函式）──
class ExamParseError(Exception): pass

def _parse_exam_xlsx(tmp):
    import openpyxl
    try:
        ws = openpyxl.load_workbook(tmp, data_only=True)["題庫"]
    except Exception:
        raise ExamParseError("讀不到「題庫」工作表")
    groups, skipped = {}, 0
...
# ── 題庫解析：Word/PDF 抽文字（掃描版 PDF 自動轉圖走 Vision）──
def _exam_extract(tmp, ext):
    if ext == "xlsx":
        import openpyxl
        wb = openpyxl.load_workbook(tmp, data_only=True)
        parts = []
        for name in wb.sheetnames:
            parts.append("【工作表：" + name + "】")
            for row in wb[name].iter_rows(values_only=True):
                cells = [str(c).strip() for c in row if c is not None and str(c).strip()]
                if cells: parts.append(" | ".join(cells))
        if len(parts) <= len(wb.sheetnames): raise ExamParseError("Excel 讀不到內容")
        return "\n".join(parts), None
    if ext == "docx":
        import docx
        d = docx.Document(tmp)
        ...
        if not parts: raise ExamParseError("Word 檔讀不到文字內容")
        return "\n".join(parts), None
    import fitz
    doc = fitz.open(tmp)
    txt = "\n".join(pg.get_text() for pg in doc)
    if len(txt.strip()) >= 100:
        doc.close(); return txt, None
    import base64
    imgs = []
    for pg in list(doc)[:10]:
        pix = pg.get_pixmap(dpi=150)
        imgs.append(base64.b64encode(pix.tobytes("png")).decode())
    doc.close()
    if not imgs: raise ExamParseError("PDF 無法讀取內容")
    return None, imgs

_EXAM_AI_PROMPT = """你是題庫整理員。從提供的考卷內容抽出所有「選擇題」，輸出唯一一個 JSON 物件、不加任何說明或 Markdown 圍欄：
{"jf":"職能關鍵字","title":"卷名","tmin":60,"questions":[{"seq":1,"prompt":"題目全文","options":["選項A內容","選項B內容","選項C內容","選項D內容"],"answer":"A","points":5}]}
規則：
- jf：%s
- title：卷內若有標題用標題，否則用「jf＋筆試」；tmin：卷內有作答時間就用，沒有填 60
- options 為 2~4 個，內容不含「A.」「(B)」等編號前綴；answer 只能是 A/B/C/D，對應 options 順序
- 卷內有標答案就照標的；完全沒有答案標示時，由你判斷正解填入
- points：卷內有配分就用，沒有填 5；seq 依卷內題號，沒有題號照出現順序
- 非選擇題（問答、實作、填空）一律跳過
- 找不到任何選擇題時輸出 {"questions":[]}"""

def _parse_exam_ai(tmp, ext, jf_hint, title_hint, tmin_hint, fname=""):
    try:
        key = env.get("ANTHROPIC_API_KEY") if hasattr(env, "get") else env["ANTHROPIC_API_KEY"]
    except Exception:
        key = None
    if not key: raise ExamParseError("尚未設定 ANTHROPIC_API_KEY（.env）")
    text, imgs = _exam_extract(tmp, ext)
    jf_rule = ("一律用「" + jf_hint + "」") if jf_hint else "從卷名/內容判斷最合適的 2~4 字職能關鍵字"
    prompt = _EXAM_AI_PROMPT % jf_rule
    import anthropic
    ...
    try:
        ...
        raise ExamParseError("AI 回傳非 JSON，請再試一次或改用 Excel 範本")
    qs, skipped = [], 0
    for i, q in enumerate(data.get("questions") or []):
        opts = [str(x).strip() for x in (q.get("options") or []) if str(x).strip()]
        ans = str(q.get("answer") or "").strip().upper()
        if not q.get("prompt") or len(opts) < 2 or ans not in list("ABCD"[:len(opts)]):
            skipped += 1; continue
        qs.append((int(q.get("seq") or i+1), str(q["prompt"]).strip(),
                   json.dumps(opts, ensure_ascii=False), ans, float(q.get("points") or 5)))
    if not qs:
        raise ExamParseError("AI 沒抽到可用的選擇題" + (("（略過 " + str(skipped) + " 題）") if skipped else ""))
    jf = (jf_hint or str(data.get("jf") or "").strip())
    if not jf: raise ExamParseError("判斷不出職能關鍵字，請在上傳時填職能")
    title = (title_hint or str(data.get("title") or "").strip() or jf + "筆試")
    try: tmin = int(tmin_hint or data.get("tmin") or 60)
...
async def exam_upload(req: Request, file: UploadFile = File(...)):
    ...
    try:
        _hints = ((qp.get("jf") or "").strip() or None,
                  (qp.get("title") or "").strip() or None,
                  (qp.get("tmin") or "").strip() or None)
        if ext == "xlsx":
            try:
                groups, skipped = _parse_exam_xlsx(tmp)
            except ExamParseError:
                groups, skipped = _parse_exam_ai(tmp, ext, *_hints, fname=(file.filename or ""))
        else:
            groups, skipped = _parse_exam_ai(tmp, ext, *_hints, fname=(file.filename or ""))
    except ExamParseError as e:
        os.remove(tmp); return JSONResponse({"ok": False, "msg": str(e)}, status_code=400)
    except Exception:
        os.remove(tmp)
        return JSONResponse({"ok": False, "msg": "解析失敗（AI 服務或檔案異常），請重試"}, status_code=500)
    os.remove(tmp)
    if not groups:
        return JSONResponse({"ok": False, "msg": "沒解析到任何題目"}, status_code=400)
    cn = db(); cur = cn.cursor()
    lines = []
...
@app.post("/api/hr/exam/clear")
def exam_clear(req: Request):
    u = me_rw(req)
    cn = db(); cur = cn.cursor()
    cur.execute("DELETE FROM rec.exam_question"); n_q = cur.rowcount
    cur.execute("DELETE FROM rec.exam_set"); n_s = cur.rowcount
    cn.commit(); cn.close()
    cn2 = db(); cur2 = cn2.cursor()
    try:
        cur2.execute("INSERT INTO rec.event(EventType, Payload) VALUES('exam_cleared', ?)", f"{n_s} sets/{n_q} questions by {u}")
        cn2.commit()
    except Exception:
        pass
    cn2.close()
    return {"ok": True, "msg": f"已清空：{n_s} 卷、{n_q} 題"}

@app.post("/api/hr/exam/delete")
def exam_delete(req: Request, jf: str = ""):
    u = me_rw(req)
    jf = (jf or "").strip()
    if not jf: raise HTTPException(400)
    cn = db(); cur = cn.cursor()
    cur.execute("SELECT Id FROM rec.exam_set WHERE JobFunction=?", jf)
    r = cur.fetchone()
    if not r:
        cn.close(); return JSONResponse({"ok": False, "msg": "找不到該卷"}, status_code=404)
    cur.execute("DELETE FROM rec.exam_question WHERE SetId=?", r.Id); n_q = cur.rowcount
    cur.execute("DELETE FROM rec.exam_set WHERE Id=?", r.Id)
    cn.commit(); cn.close()
    return {"ok": True, "msg": f"已刪除「{jf}」卷（{n_q} 題）"}

@app.get("/api/hr/exam/questions")
def exam_questions_admin(req: Request, jf: str = ""):
    u = me(req)
    cn = db(); cur = cn.cursor()
    cur.execute("SELECT Id FROM rec.exam_set WHERE JobFunction=?", (jf or "").strip())
    r = cur.fetchone()
    if not r:
        cn.close(); raise HTTPException(404)
    cur.execute("SELECT Seq, Prompt, OptionsJson, Answer, Points FROM rec.exam_question WHERE SetId=? ORDER BY Seq", r.Id)
    qs = [{"seq": x.Seq, "prompt": x.Prompt, "options": json.loads(x.OptionsJson),
           "answer": x.Answer, "points": x.Points} for x in cur.fetchall()]
    cn.close()
    return {"questions": qs}
...
async def invite(req: Request):
    ...
    if _guard_mail(cand_mail) is None:
        return JSONResponse({"ok": False, "msg": "Email 空白、格式不正確或為 104 系統信箱，請修正後再寄"}, status_code=400)
    cn = db(); cur = cn.cursor()
    ...
    cur.execute("UPDATE rec.candidate SET Email=?, Stage=?, LastContactAt=SYSDATETIME(), ContactRounds=ISNULL(ContactRounds,0)+1 WHERE Id=?", cand_mail, _stage, cid)
    if rnd >= 2:
        cur.execute("INSERT INTO rec.interview(CandidateId) VALUES(?)", cid)
...
def resume(cid: int, req: Request):
    ...
    disp = "inline" if ext in (".pdf", ".htm", ".html", ".png", ".jpg", ".jpeg") else "attachment"
    return FileResponse(fp, media_type=mt,
                        headers={"Content-Disposition": f"{disp}; filename=resume_{cid}{ext}"})
...
async def hire(req: Request):
    ...
    c = cur.fetchone()
    if not c: cn.close(); raise HTTPException(404)
    cand_mail = _guard_mail(cand_mail or (c.Email or "").strip()) or ""
    if not cand_mail:
        cn.close(); return JSONResponse({"ok": False, "msg": "此應徵者沒有有效 Email（空白／格式錯誤／104 系統信箱），請先修正"}, status_code=400)
    ...
    cur.execute("UPDATE rec.candidate SET Email=?, Stage=N'錄取', LastContactAt=SYSDATETIME() WHERE Id=?", cand_mail, cid)
    cur.execute("INSERT INTO rec.contact_log(CandidateId, Channel, Direction, Summary, Actor) VALUES(?, 'email', 'out', ?, ?)",
...
async def hire_prepare(req: Request):
    ...
    cur.execute("UPDATE rec.candidate SET Email=?, Stage=N'錄取簽核中', LastContactAt=SYSDATETIME() WHERE Id=?", cand_mail, cid)
...
async def intent(req: Request):
    ...
    if not _send_mail(cand_mail, f"Kinetics睿普工程【錄用意向確認】{c.JobTitle or ''}｜{c.Name}", body, [_logo_attachment()]):
        cn.close(); return JSONResponse({"ok": False, "msg": "寄信失敗"}, status_code=502)
    cur.execute("UPDATE rec.candidate SET Email=?, Stage=N'擬錄取待回覆', LastContactAt=SYSDATETIME(), ContactRounds=ISNULL(ContactRounds,0)+1 WHERE Id=?", cand_mail, cid)
